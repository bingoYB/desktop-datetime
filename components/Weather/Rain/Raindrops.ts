...
export default class Raindrops {
  ...
  constructor(width: number, height: number, scale: number, dropAlpha: HTMLImageElement, dropColor: HTMLImageElement, options: Partial<RaindropsOptions> = {}) {
    this.width = width;
    this.height = height;
    this.scale = scale;
  ...
  init() {
    ...
    const droplets = createCanvas(this.width * this.dropletsPixelDensity, this.height * this.dropletsPixelDensity);
    if (!droplets) throw new Error("Could not create droplets canvas");
  ...
  createDrop(options: Partial<Drop>): Drop | null {
    ...
    return Object.assign(Object.create(DropProto), options);
  }
  ...
  updateDrops(timeScale: number) {
    ...
    }); // context was this in original, converted to arrow function for `this`
